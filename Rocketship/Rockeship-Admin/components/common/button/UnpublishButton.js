...
const UnpublishButton = ({ path, action }) => {
  ...
  const onSubmit = async () => {
    try {
      await mutateAsync({ path }).then((response) => {
        if (response) {
          toast.success("This is unpublished now", {
            style: {
              padding: "8px",
              color: "#E1662C",
            },
  ...
  return (
    <>
      <button
        className="btn bg-orange w-12 h-10"
        onClick={() => {
          openModal();
        }}
      >
              ...
              <div className="inline-block w-full max-w-lg p-12 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                <div className="flex flex-col items-center justify-center">
                  <h3 className="mb-5 text-xl text-gray-800  font-medium">
                    Are you sure you want to unpublish the blog? This item will
                    be unpublished immediately. You cant undo this action.
                  </h3>
